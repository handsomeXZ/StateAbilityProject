...
FVector FCFrameGroundMoveUtils::ComputeDeflectedMoveOntoRamp(const FVector& OrigMoveDelta, const FHitResult& RampHitResult, float MaxWalkSlopeCosine, const bool bHitFromLineTrace)
{
		...
		UFloorQueryUtils::IsHitSurfaceWalkable(RampHitResult, MaxWalkSlopeCosine) && !bHitFromLineTrace)
	{
		...
		return FCFrameMovementUtils::ConstrainToPlane(OrigMoveDelta, RampSurfacePlane, true);
	}

	return OrigMoveDelta;
}

bool FCFrameGroundMoveUtils::CanStepUpOnHitSurface(const FHitResult& Hit)
{
...
bool FCFrameGroundMoveUtils::TryMoveToStepUp(USceneComponent* UpdatedComponent, UPrimitiveComponent* UpdatedPrimitive, UCFrameMoverComponent* MoverComponent, const FVector& GravDir, float MaxStepHeight, float MaxWalkSlopeCosine, float FloorSweepDistance, const FVector& MoveDelta, const FHitResult& MoveHitResult, const FFloorCheckResult& CurrentFloor, bool bIsFalling, FOptionalFloorCheckResult* OutStepDownResult)
{
	...
	{
		UE_LOG(LogCFrameGroundMove, VeryVerbose, TEXT("Not stepping up due to top of capsule hitting something"));
		return false;
	}
	...
	if (GravDir.IsZero())
	{
		UE_LOG(LogCFrameGroundMove, VeryVerbose, TEXT("Not stepping up because there's no gravity"));
		return false;
	}
	...
	if (CurrentFloor.IsWalkableFloor())
	{
		...
		StepTravelDownHeight = (MaxStepHeight + FCFrameFloorQueryUtils::MAX_FLOOR_DIST * 2.f);
	...
	{
		UE_LOG(LogCFrameGroundMove, VeryVerbose, TEXT("Not stepping up because the impact is below us"));
		return false;
	}
	...
	const bool bDidStepUp = FCFrameMovementUtils::TryMoveUpdatedComponent_Internal(UpdatedComponent, UpAdjustment, PawnRotation, true, MOVECOMP_NoFlags, &SweepUpHit, ETeleportType::None);

	UE_LOG(LogCFrameGroundMove, VeryVerbose, TEXT("TryMoveToStepUp Up: %s (role %i) UpAdjustment=%s DidMove=%i"),
		*GetNameSafe(UpdatedComponent->GetOwner()), UpdatedComponent->GetOwnerRole(), *UpAdjustment.ToCompactString(), bDidStepUp);

	if (SweepUpHit.bStartPenetrating)
	{
		// Undo movement
		UE_LOG(LogCFrameGroundMove, VeryVerbose, TEXT("Reverting step-up attempt because we started in a penetrating state"));
	...
	const bool bDidStepFwd = FCFrameMovementUtils::TryMoveUpdatedComponent_Internal(UpdatedComponent, MoveDelta, PawnRotation, true, MOVECOMP_NoFlags, &StepFwdHit, ETeleportType::None);

	UE_LOG(LogCFrameGroundMove, VeryVerbose, TEXT("TryMoveToStepUp Fwd: %s (role %i) MoveDelta=%s DidMove=%i"),
	...
	if (StepFwdHit.bBlockingHit)
	{
		...
		{
			// Undo movement
			UE_LOG(LogCFrameGroundMove, VeryVerbose, TEXT("Reverting step-fwd attempt during step-up, because we started in a penetrating state"));
			ScopedStepUpMovement.RevertMove();
			return false;
		}
		...
		{
			MoverComponent->HandleImpact(SweepUpHit, NAME_None, MoveDelta);
		}

		// pawn ran into a wall
		if (MoverComponent)
		{
			MoverComponent->HandleImpact(StepFwdHit, NAME_None, MoveDelta);
		}
		...
		const float ForwardSlideAmount = TryWalkToSlideAlongSurface(UpdatedComponent, UpdatedPrimitive, MoverComponent, MoveDelta, 1.f - StepFwdHit.Time, PawnRotation, StepFwdHit.Normal, StepFwdHit, true, MaxWalkSlopeCosine, MaxStepHeight);
		LastComponentLocation = UpdatedPrimitive->GetComponentLocation();

		if (bIsFalling)
		{
			UE_LOG(LogCFrameGroundMove, VeryVerbose, TEXT("Reverting step-fwd attempt during step-up, because we could not adjust without falling"));
			ScopedStepUpMovement.RevertMove();
			return false;
		}
		...
		{
			UE_LOG(LogCFrameGroundMove, VeryVerbose, TEXT("Reverting step-fwd attempt during step-up, because no movement differences occurred"));
			ScopedStepUpMovement.RevertMove();
			return false;
		}
	}
	...
	const bool bDidStepDown = FCFrameMovementUtils::TryMoveUpdatedComponent_Internal(UpdatedComponent, StepDownAdjustment, UpdatedComponent->GetComponentQuat(), true, MOVECOMP_NoFlags, &StepFwdHit, ETeleportType::None);

	UE_LOG(LogCFrameGroundMove, VeryVerbose, TEXT("TryMoveToStepUp Down: %s (role %i) StepDownAdjustment=%s DidMove=%i"),
	...
	if (StepFwdHit.bStartPenetrating)
	{
		UE_LOG(LogCFrameGroundMove, VeryVerbose, TEXT("Reverting step-down attempt during step-up/step-fwd, because we started in a penetrating state"));
		ScopedStepUpMovement.RevertMove();
		return false;
	}
	...
	if (StepFwdHit.IsValidBlockingHit())
	{
		...
		{
			UE_LOG(LogCFrameGroundMove, VeryVerbose, TEXT("Reject step-down attempt during step-up/step-fwd, because it made us travel too high (too high Height %.3f) up from floor base %f to %f"), DeltaZ, PawnInitialFloorBaseZ, StepFwdHit.ImpactPoint.Z);
			ScopedStepUpMovement.RevertMove();
			return false;
		}
		...
		if (!UFloorQueryUtils::IsHitSurfaceWalkable(StepFwdHit, MaxWalkSlopeCosine))
		{
			...
			if (bNormalTowardsMe)
			{
				UE_LOG(LogCFrameGroundMove, VeryVerbose, TEXT("Reject step-down attempt during step-up/step-fwd, due to unwalkable normal %s opposed to movement"), *StepFwdHit.ImpactNormal.ToString());
				ScopedStepUpMovement.RevertMove();
				return false;
			}
			...
			{
				UE_LOG(LogCFrameGroundMove, VeryVerbose, TEXT("Reject step-down attempt during step-up/step-fwd, due to unwalkable normal %s above old position)"), *StepFwdHit.ImpactNormal.ToString());
				ScopedStepUpMovement.RevertMove();
				return false;
			}
		}
		...
		{
			UE_LOG(LogCFrameGroundMove, VeryVerbose, TEXT("Reject step-down attempt during step-up/step-fwd, due to being outside edge tolerance"));
			ScopedStepUpMovement.RevertMove();
			return false;
		}
		...
		{
			UE_LOG(LogCFrameGroundMove, VeryVerbose, TEXT("Reject step-down attempt during step-up/step-fwd, due to being up onto surface with !CanStepUpOnHitSurface"));
			ScopedStepUpMovement.RevertMove();
			return false;
		}
		...
		if (OutStepDownResult != NULL)
		{
			...
			if (StepFwdHit.Location.Z > OldLocation.Z)
			{
				...
				{
					UE_LOG(LogCFrameGroundMove, VeryVerbose, TEXT("Reject step-down attempt during step-up/step-fwd, due to it being an unperchable step"));
					ScopedStepUpMovement.RevertMove();
					return false;
				}
			}
...
bool FCFrameGroundMoveUtils::TryMoveToAdjustHeightAboveFloor(USceneComponent* UpdatedComponent, UPrimitiveComponent* UpdatedPrimitive, FFloorCheckResult& CurrentFloor, float MaxWalkSlopeCosine)
{
	...
	{
		if (OldFloorDist < FCFrameFloorQueryUtils::MIN_FLOOR_DIST && CurrentFloor.LineDist >= FCFrameFloorQueryUtils::MIN_FLOOR_DIST)
		{
	...
	if (OldFloorDist < FCFrameFloorQueryUtils::MIN_FLOOR_DIST || OldFloorDist > FCFrameFloorQueryUtils::MAX_FLOOR_DIST)
	{
		FHitResult AdjustHit(1.f);
		const float InitialZ = UpdatedComponent->GetComponentLocation().Z;
		const float AvgFloorDist = (FCFrameFloorQueryUtils::MIN_FLOOR_DIST + FCFrameFloorQueryUtils::MAX_FLOOR_DIST) * 0.5f;
		const float MoveDist = AvgFloorDist - OldFloorDist;

		FCFrameMovementUtils::TrySafeMoveUpdatedComponent(UpdatedComponent, UpdatedPrimitive,
			FVector(0.f, 0.f, MoveDist), UpdatedComponent->GetComponentQuat(),
			true, AdjustHit, ETeleportType::None);
...
float FCFrameGroundMoveUtils::TryWalkToSlideAlongSurface(USceneComponent* UpdatedComponent, UPrimitiveComponent* UpdatedPrimitive, UCFrameMoverComponent* MovementComponent, const FVector& Delta, float PctOfDeltaToMove, const FQuat Rotation, const FVector& Normal, FHitResult& Hit, bool bHandleImpact, float MaxWalkSlopeCosine, float MaxStepHeight)
{
	if (!Hit.bBlockingHit)
	{
		return 0.f;
	...
	FVector SlideDelta = FCFrameMovementUtils::ComputeSlideDelta(Delta, PctOfDeltaToMove, SafeWalkNormal, Hit);
	...
	if (SlideDelta.Dot(Delta) > 0.f)
	{
		...
		if (Hit.IsValidBlockingHit())
		{
			...
			{
				MovementComponent->HandleImpact(Hit, NAME_None, SlideDelta);
			}

			// Compute new slide normal when hitting multiple surfaces.
			SlideDelta = FCFrameMovementUtils::ComputeTwoWallAdjustedDelta(SlideDelta, Hit, OldSafeHitNormal);
